...
class MRF(object):
    def __init__(self, model, optimizer, content_weight, style_weight, tv_weight, content_layers, style_layers, style_id, resolution_num=1, device_id=-1, initial_image='content', keep_color=False):
        self.style_id = style_id.split("/")[-1].split(".")[0]
        self.model = model
        self.optimizer = optimizer
        self.content_weight = content_weight
        self.style_weight = style_weight
